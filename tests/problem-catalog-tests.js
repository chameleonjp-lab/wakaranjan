...
test('点数問題の正解番号が計算結果の選択肢を指す',()=>{for(const q of catalog.questions.filter(q=>q.category==='score'))assert.match(q.choices[q.answerIndex],new RegExp(String(q.expectedTotal)),`${q.id}: answerIndex=${q.answerIndex}`)});
