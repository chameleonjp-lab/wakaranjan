...
test('全問題IDが横断して重複しない',()=>{
  const ids=all.map(q=>q.id);assert.equal(new Set(ids).size,ids.length);
});

test('全問題の選択肢は正規化後も重複しない',()=>{
  for(const q of all){const choices=q.choices||[];assert.ok(choices.length>=2,q.id);const normalized=choices.map(norm);assert.equal(new Set(normalized).size,normalized.length,`${q.id}: ${choices.join(' / ')}`)}
});

test('正解番号と解説が教材として最低限成立する',()=>{
  for(const q of all){assert.ok(Number.isInteger(q.answerIndex)&&q.answerIndex>=0&&q.answerIndex<q.choices.length,q.id);assert.ok((q.prompt||'').length>=8,`${q.id}: prompt`);assert.ok((q.explanation||'').length>=10,`${q.id}: explanation`);assert.notEqual(norm(q.explanation),norm(q.choices[q.answerIndex]),`${q.id}: explanation only repeats answer`)}
});
...
test('対々和問題は四暗刻との別解が生まれない問い方になっている',()=>{
  const q=problemHub.find(q=>q.id==='q-yaku-007');assert.ok(q);assert.match(q.prompt,/鳴いて/);assert.match(q.prompt,/2翻/);assert.match(q.explanation,/四暗刻/);
});

test('45萬と46萬の牌効率問題は残り枚数の前提を明示する',()=>{
  const q=problemHub.find(q=>q.id==='q-practical-003');assert.ok(q);assert.match(q.prompt,/見えている牌の枚数が同じ/);assert.match(q.explanation,/実戦/);assert.match(q.explanation,/見えている枚数/);
});

test('ルール差問題はサイト標準か一般差かを設問内で区別する',()=>{
  for(const q of practical.filter(q=>q.category==='rule-diff')){
