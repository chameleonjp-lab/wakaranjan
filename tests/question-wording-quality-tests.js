...
test('待ち・役名・点数計算は全問で選択肢別説明を持つ',()=>{const core=problemHub.filter(q=>['ron-wait','yaku-name','score'].includes(q.category));assert.ok(core.length>=48,'core question coverage');for(const q of core){assert.equal(q.choiceFeedback?.length,q.choices.length,`${q.id}: missing generated feedback`);for(const text of q.choiceFeedback)assert.ok(text.length>=12,`${q.id}: feedback too short`)}});
