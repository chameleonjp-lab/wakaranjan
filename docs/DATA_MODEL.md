...
捨て牌後の応答待ちでは、捨てた牌を一時的に保持する。ロン判定はこの状態から行い、見送った場合だけ通常のツモ待ちへ移る。判定には共通の門前・副露対応あがり計算、フリテン、親位置、本場、供託を渡す。ツモ判定は現在の手番で捨てる前に行う。

通常の牌山が尽きた場合は途中状態を変更せず結果待ちへ移り、テンパイ情報を確定したあと流局として局を完了する。完了時は roundState の局進行・連荘・本場処理へ結果を渡し、次の局を開始するまで実牌進行を再開できない。
