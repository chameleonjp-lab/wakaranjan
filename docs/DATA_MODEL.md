...
- currentSeat: 現在、ツモまたは捨て牌を行う席
- phase: ツモ待ち、捨て牌待ち、結果待ち、完了

配牌直後は親が14枚、他家が13枚で、最初のドラ表示牌を1枚開く。親の第1打が終わると、次の席が通常の牌山から1枚ツモり、捨て牌を終えて次の席へ進む。

カンは手牌だけを先に変えて終わりにしない。牌山側で嶺上牌と追加ドラ表示牌を同時に確保し、局状態にも同じ結果を保存する。通常の牌山または嶺上牌が不足する場合は、途中まで進めず、元の状態を残す。

この状態層は、あがり形の判定や他家の自動思考とは分ける。まず牌の移動と手番の順序を検査し、その後にあがり・鳴き・流局の判断を接続する。
