...
捨て牌後の応答待ちでは、捨てた牌を一時的に保持する。ロン判定はこの状態から行い、見送った場合だけ通常のツモ待ちへ移る。複数の席を同時に判定できるようにし、成立したロン候補を放銃者の次に近い席から選ぶ。これを頭ハネとし、選ばれなかった候補も `blockedRonClaimants` として結果へ保存する。得点移動と局の勝者は、頭ハネで選ばれた1人だけに適用する。大明槓が成立した場合は、手元の同じ牌3枚を公開面子に移し、捨て牌をその面子へ取り込み、嶺上牌と追加ドラ表示牌を確定して、鳴いた人の捨て牌待ちへ移る。

あがり判定には共通の門前・副露対応あがり計算、フリテン、親位置、本場、供託を渡す。ツモ判定は現在の手番で捨てる前に行う。
