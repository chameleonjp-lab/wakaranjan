...
function load(){try{return JSON.parse(localStorage.getItem(KEY)||'{"completed":[],"lastLesson":null}')}catch{return {completed:[],lastLesson:null}}}
function save(v){try{localStorage.setItem(KEY,JSON.stringify(v))}catch{}}
export function getLessonProgress(){const p=load();return {completed:new Set(p.completed||[]),lastLesson:p.lastLesson||null}}
