...
function levelName(level){return {intro:'入門',beginner:'初級',intermediate:'中級',advanced:'上級',special:'特例・ルール差編'}[level]||level}
...
function renderHome(ctx){
  ...
  const last=ctx.lessonById.get(p.lastLesson);const firstIncomplete=ctx.lessons.find(l=>!p.completed.has(l.id));const next=last&&!p.completed.has(last.id)?last:firstIncomplete;
  app.innerHTML=`<section class="hero"><div class="eyebrow">麻雀を知らなくても大丈夫</div><h1>牌を触りながら、少しずつ覚える。</h1><p>「1枚取る、1枚捨てる」から始め、役と点数まで順番に進みます。</p>${next?`<div class="action-row"><a class="primary" href="#${next.id}">続きから：${next.title}</a></div>`:''}</section>
  <section class="panel"><h2>学習の進み具合</h2><div class="progress-summary"><span>入門 ${progressSummary(intro,p)}</span><span>初級 ${progressSummary(beginner,p)}</span><span>中級 ${progressSummary(intermediate,p)}</span><span>上級 ${progressSummary(advanced,p)}</span><span>特例 ${progressSummary(special,p)}</span></div><div class="action-row"><button id="clear-lesson-progress" class="secondary" type="button">教材の進捗を消す</button></div></section>
  ...
  <h3>中級：点数計算の基礎 <small>${progressSummary(intermediate,p)}</small></h3>${renderLessonList(intermediate,p)}<a class="lesson-card" href="#intermediate-review"><strong>中級 点数計算の総復習</strong><small>翻・符・親子・ロン/ツモ・満貫以上</small></a>
