...
export function renderFullRoundPractice(app,ctx){
  ...
  const render=()=>{
    ...
    app.innerHTML=`<section class="lesson-head"><div class="eyebrow">通し型実戦練習 ${scenarioIndex+1} / ${FULL_ROUND_SCENARIOS.length}</div><h1>${scenario.title}</h1><p class="lead">${scenario.description}</p></section><section class="practice-surface"><div class="review-progress"><span style="width:${stepIndex/scenario.steps.length*100}%"></span></div><p class="status">${stepIndex+1} / ${scenario.steps.length}　${step.title}</p><p class="status">前の判断：${stateLabels.length?stateLabels.join('・'):'まだ成立していません'}</p><p>${step.prompt}</p><div class="selection-area selection-area-choices"><h3>選択肢</h3><div class="practice-options" id="round-options"></div></div><div class="selection-area selection-area-hand" ${hasVisual?'':'hidden'}><h3>手牌・場面</h3><div id="round-state"></div></div><div class="feedback" id="round-feedback" aria-live="polite"></div><div class="action-row" id="round-actions"></div></section><div class="lesson-nav"><a class="secondary" href="#practice">対局練習へ戻る</a><a class="secondary" href="#menu">メニューへ</a></div>`;
