...
export const FULL_ROUND_SCENARIOS=[
  {id:'speed-riichi',title:'東1局：門前で速度を作る',description:'配牌からリーチ、終盤のロン判断までを連続して考えます。',steps:[
    ...
    {title:'危険牌を引く',hand:['2m','3m','4m','4p','5p','6p','7p','8p','2s','3s','4s','2z','2z'],draw:'9m',prompt:'リーチ後に9萬をツモしました。あがり牌ではありません。どうしますか？',choices:['9萬をそのまま切る','手牌を自由に組み替える','リーチを取り消す'],answer:0,feedback:['リーチ後は原則として手牌を自由に変えず、あがり牌でなければツモ牌を切ります。','リーチ後は自由な手替わりができません。','成立したリーチを途中で取り消すことはできません。']},
    {title:'ロン判断',hand:['2m','3m','4m','4p','5p','6p','7p','8p','2s','3s','4s','2z','2z'],opponentDiscard:'9p',prompt:'他家が待ち牌の9筒を捨てました。自分の河に待ち牌はありません。どうしますか？',choices:['ロンする','フリテンなので見送る','ポンする'],answer:0,feedback:['役のあるリーチ手で、フリテンでもないためロンできます。','自分の河に待ち牌がないので、この条件ではフリテンではありません。','あがれる牌ならロンを優先して局を終えます。']}
  ]},
  {id:'call-or-close',title:'東2局：鳴くか門前を保つか',description:'鳴きの条件と、鳴いた後に失う役を続けて確認します。',steps:[
    ...
    {title:'鳴いた後',hand:['6m','7m','8m','2p','3p','4p','5s','5s','6s','7s','8s'],meld:['3m','4m','5m'],prompt:'チーした後、この手は門前ですか？',choices:['門前ではない','まだ門前','暗槓と同じ扱い'],answer:0,feedback:['チーは副露なので門前ではなくなります。','チーすると手の一部を公開するため門前ではありません。','暗槓は門前を保ちますが、チーは違います。']},
    {title:'リーチ可否',hand:['6m','7m','8m','2p','3p','4p','5s','5s','6s','7s','8s'],meld:['3m','4m','5m'],prompt:'その後テンパイしました。リーチできますか？',choices:['できない','できる','1000点棒を2本出せばできる'],answer:0,feedback:['チーで門前ではなくなったためリーチできません。','リーチは門前が条件です。','供託する点数を増やしても門前条件は変わりません。']},
    {title:'あがり役を確認',hand:['6m','7m','8m','2p','3p','4p','5s','5s','6s','7s','8s'],meld:['3m','4m','5m'],prompt:'鳴いた手であがるには何を確認しますか？',choices:['鳴いても成立する役があるか','ドラが1枚あれば必ずあがれる','門前役が残っているかだけ'],answer:0,feedback:['鳴いた後は、鳴いても成立する役があるかを確認します。','ドラは役ではないので、ドラだけではあがれません。','門前限定役は使えません。鳴いて成立する役を探します。']},
    {title:'局の振り返り',prompt:'この一局で最も重要な因果関係は？',choices:['チーした→門前でなくなった→リーチ不可','チーした→必ず点数が上がる','チーした→フリテンが必ず解除される'],answer:0,feedback:['前の判断が後の選択肢を変える例です。鳴くと速度は上がることがありますが、門前限定役を失います。','鳴くと役や翻数が下がる場合もあります。','鳴いたこと自体でフリテンが必ず解除されるわけではありません。']}
  ]},
  {id:'late-defense',title:'東4局：終盤の押し引き',description:'順位・手の価値・危険度を一つずつ確認して終盤判断をします。',steps:[
    ...
    {title:'自分の手',hand:['2m','3m','4m','4p','5p','6p','3s','4s','5s','7s','8s','2z','2z'],prompt:'まだ1シャンテンで、あがりまで距離があります。安全な6萬を持っているなら？',choices:['いったん6萬を切って守備寄りに進める','危険牌から先に切る','必ずカンする'],answer:0,feedback:['固定例では、手が遠いため現物を使って放銃を避けます。','危険牌を先に切る根拠はありません。','カンできる牌姿でもなく、守備目的の選択にもなりません。']},
...
function tileRow(ctx,codes,label){if(!codes?.length)return null;const box=document.createElement('div');box.className='practice-tile-block';const strong=document.createElement('strong');strong.textContent=label;const row=document.createElement('div');row.className='tile-row';for(const code of codes){const el=tile(ctx,code);if(el)row.append(el)}box.append(strong,row);return box}

export function renderFullRoundPractice(app,ctx){
  let scenarioIndex=0,stepIndex=0,score=0,answered=false;const history=[];
  const render=()=>{
    const scenario=FULL_ROUND_SCENARIOS[scenarioIndex];const step=scenario.steps[stepIndex];
    if(!step){
      ...
      app.querySelector('#next-scenario').onclick=()=>{scenarioIndex=(scenarioIndex+1)%FULL_ROUND_SCENARIOS.length;stepIndex=0;score=0;history.length=0;answered=false;render()};return;
    }
    app.innerHTML=`<section class="lesson-head"><div class="eyebrow">通し型実戦練習 ${scenarioIndex+1} / ${FULL_ROUND_SCENARIOS.length}</div><h1>${scenario.title}</h1><p class="lead">${scenario.description}</p></section><section class="practice-surface"><div class="review-progress"><span style="width:${stepIndex/scenario.steps.length*100}%"></span></div><p class="status">${stepIndex+1} / ${scenario.steps.length}　${step.title}</p><div id="round-state"></div><p>${step.prompt}</p><div class="practice-options" id="round-options"></div><div class="feedback" id="round-feedback" aria-live="polite"></div><div class="action-row" id="round-actions"></div></section><div class="lesson-nav"><a class="secondary" href="#practice">対局練習へ戻る</a><a class="secondary" href="#home">ホームへ</a></div>`;
    const state=app.querySelector('#round-state');for(const [codes,label] of [[step.hand,'あなたの手牌'],[step.meld,'公開した組'],[step.river,'相手の河']]){const block=tileRow(ctx,codes,label);if(block)state.append(block)}if(step.draw){const block=tileRow(ctx,[step.draw],'ツモ牌');if(block)state.append(block)}if(step.opponentDiscard){const block=tileRow(ctx,[step.opponentDiscard],'他家の捨て牌');if(block)state.append(block)}
