...
function renderDrawDiscard(app,ctx){
  ...
  const render=()=>{
    app.innerHTML='<section class="lesson-head"><div class="eyebrow">対局練習 1</div><h1>ツモと捨て牌</h1><p class="lead">自分の番は、山から1枚取って、手牌から1枚捨てます。</p></section><section class="practice-surface"><p id="draw-message" class="status"></p><div class="practice-step-list"><span class="active">1 ツモ</span><span class="'+(state!=='ready'?'active':'')+'">2 捨てる</span><span class="'+(state==='done'?'active':'')+'">3 確認</span></div><h2 class="section-title">あなたの手牌</h2><div class="practice-hand" id="draw-hand"></div><h2 class="section-title">自分の河</h2><div class="river" id="draw-river"></div><div class="action-row" id="draw-actions"></div><div class="feedback" id="draw-feedback"></div></section>'+nav();
    ...
    appendTileRow(handBox,hand.map(code=>ctx.tileByCode.get(code)),{rowClass:state==='drawn'?'practice-discard-row':'hand-fit-row',interactive:state==='drawn',drawnIndex:state==='drawn'?hand.length-1:-1,onSelect:(_tile,_options,_element,index)=>{if(state==='drawn'){river.push(hand[index]);hand.splice(index,1);state='done';render()}}});
