...
export function renderDataLesson(app,ctx,lesson){
  ...
  const prev=neighbor(ctx,lesson,-1),next=neighbor(ctx,lesson,1);const nav=document.createElement('div');nav.className='lesson-nav';nav.innerHTML=`${prev?`<a class="secondary" href="#${prev.id}">前へ：${prev.title}</a>`:'<a class="secondary" href="#home">一覧へ戻る</a>'}${next?`<a class="primary" href="#${next.id}">次へ：${next.title}</a>`:'<a class="primary" href="#home">一覧へ戻る</a>'}`;app.append(nav);
}
