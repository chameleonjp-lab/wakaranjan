...
export function appendTileRow(container,items,options={}){const row=document.createElement('div');row.className=['tile-row',options.rowClass||''].filter(Boolean).join(' ');const tileOptions={...options};delete tileOptions.rowClass;items.forEach((item,index)=>{const isPhysical=item&&typeof item==='object'&&Object.prototype.hasOwnProperty.call(item,'tile');const tile=isPhysical?item.tile:item;const red=isPhysical&&Boolean(item.red);row.append(createTile(tile,{...tileOptions,red,drawn:options.drawnIndex===index,onSelect:options.onSelect?((t,o,el)=>options.onSelect(t,o,el,index)):undefined}))});container.append(row);return row}
