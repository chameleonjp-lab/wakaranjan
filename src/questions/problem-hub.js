...
export function clearProblemStats(){try{localStorage.removeItem(STATS_KEY)}catch{}}
export function clearProblemStudyRecord(){clearProblemWrong();clearProblemStats()}
...
export function renderProblemHub(app,ctx){
  ...
  <section class="panel"><h2>間違えた問題</h2><p>${wrong.size?`${wrong.size}問が復習待ちです。正解するまで残ります。`:'現在、復習待ちの問題はありません。'}</p><div class="action-row"><button id="wrong-review" class="primary" type="button" ${wrong.size?'':'disabled'}>間違えた問題だけ解く</button>${wrong.size?'<button id="clear-wrong" class="secondary" type="button">復習記録を消す</button>':''}<button id="clear-stats" class="secondary" type="button">正答記録を消す</button></div></section>
