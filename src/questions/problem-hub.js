...
function renderTileRow(codes,ctx,label,{focusCodes=[],focusLabel=''}={}){if(!codes?.length)return null;const wrap=document.createElement('div');wrap.className='visual-question-block';if(label){const h=document.createElement('strong');h.textContent=label;wrap.append(h)}if(focusLabel){const note=document.createElement('p');note.className='visual-focus-note';note.textContent=`点線の牌：${focusLabel}`;wrap.append(note)}const row=document.createElement('div');row.className='tile-row visual-question-tiles no-scroll-hand hand-fit-row';for(const code of codes){const tile=ctx.tileByCode.get(code);if(tile){const node=createTile(tile,{interactive:false});node.dataset.tileCode=code;if(focusCodes.includes(code)){node.dataset.focus='true';node.setAttribute('data-focus-label',focusLabel==='待ちの形'?'待ち':'形')}row.append(node)}}wrap.append(row);return wrap}
