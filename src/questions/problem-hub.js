...
export function renderProblemHub(app,ctx){
  ...
  app.innerHTML=`<section class="hero"><div class="eyebrow">問題</div><h1>覚えた内容を、問題で確かめる。</h1><p>文章問題に加え、実際の牌姿を見て答える問題も出題します。学習結果は名前をキーにSupabaseへ保存し、通信できないときは端末内の記録で続けます。</p></section>
...
export function renderProblemSession(app,ctx,{category=null,wrongOnly=false,adaptive=false}={}){
  ...
  const session=adaptive?adaptiveSample(pool,size,stats,misconceptions):shuffled(pool).slice(0,size);let index=0,correct=0,answered=false;
  const cat=data.categories.find(c=>c.id===category);const title=wrongOnly?'間違えた問題の復習':adaptive?'苦手・勘違い優先練習':cat?.name||'問題';
  const render=()=>{
    scrollAppToTop();
    ...
    app.innerHTML=`<section class="lesson-head"><div class="eyebrow">${title}</div><h1>問題 ${index+1} / ${session.length}</h1><p class="lead">${q.prompt}</p></section><section class="panel"><div class="review-progress"><span style="width:${index/session.length*100}%"></span></div><div class="problem-choice-area"><h3>選択肢</h3><div class="quiz-options" id="problem-options"></div></div><div class="problem-hand-area" ${q.presentation==='tiles'?'':'hidden'}><h3>手牌・場面</h3><div id="problem-visual"></div></div><div class="feedback" id="problem-feedback" aria-live="polite"></div><div class="action-row" id="problem-actions"></div></section>`;
