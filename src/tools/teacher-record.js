...
export function renderTeacherRecord(app,ctx){
  ...
  app.innerHTML=`<section class="lesson-head"><div class="eyebrow">家庭・先生向け</div><h1>学習状況の確認</h1><p class="lead">${escapeHtml(profile?.name||'この学習者')}さんの記録を確認する画面です。名前を同期キーにしてSupabaseへ保存し、ゲームスコアとは別に管理します。</p><p class="sync-status" data-sync-state="${escapeHtml(sync.state)}" role="status" aria-live="polite">${escapeHtml(sync.message)}</p></section>
